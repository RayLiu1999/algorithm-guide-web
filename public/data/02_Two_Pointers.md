...
- **💡 白話文解說**：和 Two Sum 非常相似，只是這次我們要找三個數字相加剛好等於零。先排序，固定第一個數字，用左右指標去找另外兩個。如果三數之和大於零，右指標往左移（變小）；如果小於零，左指標往右移（變大）；如果剛好等於零就記錄答案，並跳過剩下的重複元素。
- **要點**：跳過重複的值避免重複答案。
- **複雜度**：O(n²) / O(1)
...
- **💡 白話文解說**：每個回文都像一顏洋譔，從中心向外一層層展開。我們把每個字元（或每兩个相鄰字元）都試著當作回文的「中心」，然後將左右兩邊同步擴展；只要左右兩個字元相同就繼續展開，不匹配就停止。過程中記錄見過最長的回文區段。
- **複雜度**：O(n²) / O(1)
