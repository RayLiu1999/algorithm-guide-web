...
const MarkdownViewer = ({ content, categoryId }: MarkdownViewerProps) => {
  ...
  const components: Components = useMemo(() => ({
    ...
    h3: ({ children, id }) => {
      ...
      return (
        <h3 id={id} className="text-xl font-semibold text-indigo-300 mt-8 mb-4 flex items-center gap-2">
          <span>{children}</span>
          {isMastered && (
            <span className="shrink-0 inline-flex items-center gap-1 text-[12px] font-semibold text-emerald-300 bg-emerald-950/70 border border-emerald-500/30 px-2 py-0.5 rounded-full ml-1">
